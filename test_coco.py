...
if __name__ == "__main__":
    ...
    valid_dataset = COCODataset(root=dataroot, type='val')
    ...
    for i, (images_batch, annotations_batch) in enumerate(valid_data_loader):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(len(images_batch)):
                boxes, labels, mask = annotations[batch_idx]["boxes"], annotations[batch_idx]["labels"].cpu(), annotations[batch_idx]["masks"].cpu()
                ...
                # cv2.imwrite("visualization/" + str(count) + "_annot_masks.jpg", cv_mask)
